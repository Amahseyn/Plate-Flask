...
def get_all_plates():
    try:
        ...
        # Prepare the list of plates with mine names
        plates_list = []
        for row in plates:
            camera_id = row[6]  # Extract camera_id
            cursor.execute("SELECT mine_name FROM mine_info WHERE mine_id = %s", (camera_id,))
            mine_name_result = cursor.fetchone()
            mine_name = mine_name_result[0] if mine_name_result else None
            ...
            plates_list.append({
                ...
                "permit": row[5],
                "mine_name": mine_name  # Include mine_name in the response
            })
...
def create_mine():
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s)
            RETURNING mine_id
        """, (data['mine_name'], data.get('location'), data.get('owner_name'), data.get('contact_number')))
...
def update_mine(mine_id):
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE mine_id = %s 
        """, (data['mine_name'], data.get('location'), data.get('owner_name'), data.get('contact_number'), mine_id))
        conn.commit()
    ...
    try:
        ...
        if page == 0:
            query = """
                ...
                WHERE mine_name ILIKE %s OR location ILIKE %s OR owner_name ILIKE %s ORDER BY mine_id;
            """
        ...
        query = """
            ...
            LIMIT %s OFFSET %s ORDER BY mine_id;
        """
        count_query = """
